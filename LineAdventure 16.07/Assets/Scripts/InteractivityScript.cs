...
public class InteractivityScript : MonoBehaviour{
    ...
    void Start()
    {
        DATA = (Hashtable)DataThread.GetData(new MemoryStream(fileToLoad.bytes));
        count = (int)(DATA[CONST.DATA_DATA] as Hashtable)[CONST.DATA_COUNT];
        attack = gameObject.GetComponent<AttackerScript>();
    }
    ...
    public void setAttackState(bool t)
    {
        isAttacking = t;
    }
