...
public class MainGameScript : MonoBehaviour {
    ...
    void Tick()
    {
        ...
        if (t.deltaPosition.x == 0 && t.deltaPosition.y == 0) return;
        ...
        addVertex();
        //PROJECTILE COLLISION CHECK - TODO


        //ENEMY COLLISION CHECK - TODO
    ...
    private bool isCollidinWithVertices(Vector2[] enemyVertices)
    {
        ...
        if (TotalLines2 > 1)
        {
            for (int i = 0; i < TotalLines2; i++)
            {
                if (i + 1 >= TotalLines2)
                {
                    lines2[i].StartPoint = (Vector3)enemyVertices[i];
                    lines2[i].EndPoint = (Vector3)enemyVertices[0];
                }
                else {
                    lines2[i].StartPoint = (Vector3)enemyVertices[i];
                    lines2[i].EndPoint = (Vector3)enemyVertices[i + 1];
                }
            }
        }
        myLine lastline;
        lastline.StartPoint = vertices[vertices.Count - 2];
        lastline.EndPoint = vertices[vertices.Count - 1];
    ...
    private bool isLineCollideHashtable()
    {
        ...
        {
            myLine currentLine;
            currentLine.StartPoint = (Vector3)vertices[vertices.Count - 2];
            currentLine.EndPoint = (Vector3)vertices[vertices.Count - 1];
            if (isLinesIntersect(lines[i], currentLine)) //CLOSED
            {
                hash[START_LINE] = lines[i];
                hash[END_LINE] = currentLine;
                return true;
