...
public class MainGameScript : MonoBehaviour {
	...
	void Updates () {
        ...
        if (touches.Length != 0)
        {
           ...
           // Debug.Log(t.phase);
            ...
            {
                //toDraw.AddRange(getPoints(list[list.Count - 1], vec, 0.5f));
                toDraw.Add(vec);
            }
            addVertex();
    ...
    public void drawLine()
    {
       ...
       // Debug.Log(list.Count);
        ...
        {
            GL.Vertex3(toDraw[i-1].x, toDraw[i-1].y, 0);
           // GL.Vertex3(0, 0, 0);
            // Another vertex at edge of circle
            GL.Vertex3(toDraw[i].x, toDraw[i].y, 0f);
        }
        GL.End();
        GL.PopMatrix();
    }
    ...
    private void isInside(myLine startLine, myLine endLine)
    {
        ...
        foreach (Enemy obj in enemies)
        {
            ...
            if (areAllPointsInThePolygon(ver, list.ToArray().toVector2Array()))
             {
