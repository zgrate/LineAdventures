...
public static class DataThread {
    ...
    public static void SaveDataUsingXML(object obj, string directory, string filename)
    {
        ...
        wr.Close();
        string final = CesarEncrypt(s, 1);
        File.WriteAllText(directory + filename, final);
    }

    public static object ReadDataUsingXML(string directory, string filename, Type type)
    {
        string s = File.ReadAllText(directory + filename);
        XmlSerializer ser = new XmlSerializer(type);
        return ser.Deserialize(new StringReader(CesarDecrypt(s, 1)));
    }
    private static string CesarDecrypt(string s, int move)
    {
        StringBuilder builder = new StringBuilder();
        foreach (char c in s.ToCharArray())
        {
            builder.Append((char)(c - move));
        }
        return builder.ToString();
    }
    private static string CesarEncrypt(string s, int move)
    {
        StringBuilder builder = new StringBuilder();
        foreach(char c in s.ToCharArray())
        {
