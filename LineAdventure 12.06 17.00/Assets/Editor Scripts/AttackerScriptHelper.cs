...
public class AttackerScriptHelper : MonoBehaviour {
    ...
    public void Update()
    {
        ...
        //Gizmos.DrawLine(new Vector3(0, 0), new Vector3(1, 1));
    }
        ...
        if (GameObject != null)
        {
            foreach (AttackData data in attackdatas)
            {
                if (data.type == AttackerScript.AttackType.VERTEX_SHOOTER)
                {
                    ...
                    {
                        Gizmos.DrawLine(GameObject.transform.TransformPoint((Vector2)hash[AttackerScript.CONST.VERTEX_DATA_VERTEX_MAIN]), GameObject.transform.TransformPoint((Vector2)hash[AttackerScript.CONST.VERTEX_DATA_VERTEX_SECOND]));
                    }
                }
            }
        }
