            ...
            if(attackData.type != AttackerScript.AttackType.IDLE)
            {
                ...
                if (attackData.type == AttackerScript.AttackType.VERTEX_SHOOTER)
                {
                    ...
                    for(int a = 0; i < table.Count; i++)
                    {
                        ...
                        ht[AttackerScript.CONST.VERTEX_DATA_VERTEX_SECOND] = ((Vector2)EditorGUILayout.Vector2Field("Second Vector", (Vector2)ht[AttackerScript.CONST.VERTEX_DATA_VERTEX_SECOND]));
